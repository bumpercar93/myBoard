...
$(document).ready(function() {
	...
	$(".deleteBtn").on("click", function() {
		...
		if($("#file_id1").val() == "") {
			$("#file_id1").val(fileid);
		}else if($("#file_id2").val() == "") {
			$("#file_id2").val(fileid);
		}else if($("#file_id3").val() == "") {
			$("#file_id3").val(fileid);
		}else if($("#file_id4").val() == "") {
			$("#file_id4").val(fileid);
		}else if($("#file_id5").val() == "") {
			$("#file_id5").val(fileid);
		}
		$(this).parent().remove();
...
// 		$("#frm").submit();
	...
	$("#addFileBtn").on("click", function() {
		...
		if($("#file1").css("display") == "none") {
			...
			cnt++;
		}else if($("#file2").css("display") == "none") {
			$("#file2").css("display", "block");
			cnt++;
		}else if($("#file3").css("display") == "none") {
			$("#file3").css("display", "block");
			cnt++;
		}else if($("#file4").css("display") == "none") {
			$("#file4").css("display", "block");
			cnt++;
		}else if($("#file5").css("display") == "none") {
						...
						<form action="${cp}/insertPost" 
							...
							<c:forEach items="${myfileList}" var="myfileVO">
								<label class="filelable">${myfileVO.myfile_name} 
									<button type="button" class="deleteBtn" data-fileid="${myfileVO.myfile_id}">삭제</button>
									<input type="hidden" value="${myfileVO.myfile_id}"/>
								</label>
								<br>
							</c:forEach>
										
							<input type="hidden" name="file_id" id="file_id1"/>
							<input type="hidden" name="file_id" id="file_id2"/>
							<input type="hidden" name="file_id" id="file_id3"/>
							<input type="hidden" name="file_id" id="file_id4"/>
							<input type="hidden" name="file_id" id="file_id5"/>
							<input type="file" class="files" id="file1" name="file1"/>
							<input type="file" class="files" id="file2" name="file2"/>
							<input type="file" class="files" id="file3" name="file3"/>
							<input type="file" class="files" id="file4" name="file4"/>
							<input type="file" class="files" id="file5" name="file5"/>
